...
export function CheckoutForm({ product }: { product: CheckoutProduct }) {
  ...
  const upiUrl = `upi://pay?pa=petbot%40ptyes&pn=Pet%20bot&am=${product.price.toFixed(2)}&cu=INR`;
  if (order) return <main className="checkout-page"><section className="checkout-success"><p className="eyebrow">Order created</p><h1>Thank you.</h1><p>Your order <strong>{order.orderNumber}</strong> is reserved. When you are ready, tap below to open the secure UPI payment step.</p>{!paymentOpen ? <button type="button" onClick={() => setPaymentOpen(true)} className="button button-dark payment-complete">Pay via UPI →</button> : <><div className="upi-panel"><img src="/media/petbot-upi-qr.jpeg" alt="Pet bot UPI payment QR code" /><div><p className="eyebrow">Manual UPI payment</p><strong>₹{product.price.toFixed(2)}</strong><span>Pet bot · Kanishka Dwivedi</span><a className="button button-dark" href={upiUrl}>Open UPI app ↗</a></div></div><button type="button" onClick={markPaymentCompleted} className="button button-dark payment-complete">I’ve completed payment</button>{paymentMessage && <p className="payment-message" role="status">{paymentMessage}</p>}</>}<Link href="/" className="text-link">← Return to Petbot</Link></section></main>;
